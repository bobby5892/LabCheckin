 ...
 *
 * @method     ChildCourseQuery leftJoinLabVisit($relationAlias = null) Adds a LEFT JOIN clause to the query using the LabVisit relation
 * @method     ChildCourseQuery rightJoinLabVisit($relationAlias = null) Adds a RIGHT JOIN clause to the query using the LabVisit relation
 * @method     ChildCourseQuery innerJoinLabVisit($relationAlias = null) Adds a INNER JOIN clause to the query using the LabVisit relation
 *
 * @method     ChildCourseQuery joinWithLabVisit($joinType = Criteria::INNER_JOIN) Adds a join clause and with to the query using the LabVisit relation
 *
 * @method     ChildCourseQuery leftJoinWithLabVisit() Adds a LEFT JOIN clause and with to the query using the LabVisit relation
 * @method     ChildCourseQuery rightJoinWithLabVisit() Adds a RIGHT JOIN clause and with to the query using the LabVisit relation
 * @method     ChildCourseQuery innerJoinWithLabVisit() Adds a INNER JOIN clause and with to the query using the LabVisit relation
 *
 * @method     \LabVisitQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
...
abstract class CourseQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT id, name FROM course WHERE id = :p0';
        try {
    ...
    /**
     * Filter the query by a related \LabVisit object
     *
     * @param \LabVisit|ObjectCollection $labVisit the related object to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return ChildCourseQuery The current query, for fluid interface
     */
    public function filterByLabVisit($labVisit, $comparison = null)
    {
        if ($labVisit instanceof \LabVisit) {
            return $this
                ->addUsingAlias(CourseTableMap::COL_ID, $labVisit->getCourseid(), $comparison);
        } elseif ($labVisit instanceof ObjectCollection) {
            return $this
                ->useLabVisitQuery()
                ->filterByPrimaryKeys($labVisit->getPrimaryKeys())
                ->endUse();
        } else {
            throw new PropelException('filterByLabVisit() only accepts arguments of type \LabVisit or Collection');
        }
    }

    /**
     * Adds a JOIN clause to the query using the LabVisit relation
     *
     ...
     * @return $this|ChildCourseQuery The current query, for fluid interface
     */
    public function joinLabVisit($relationAlias = null, $joinType = Criteria::LEFT_JOIN)
    {
        $tableMap = $this->getTableMap();
        $relationMap = $tableMap->getRelation('LabVisit');
        ...
        if ($relationAlias) {
            ...
            $this->addJoinObject($join, 'LabVisit');
        }
    ...
    /**
     * Use the LabVisit relation LabVisit object
     *
     * @see useQuery()
     *
     * @param     string $relationAlias optional alias for the relation,
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \LabVisitQuery A secondary query class using the current class as primary query
     */
    public function useLabVisitQuery($relationAlias = null, $joinType = Criteria::LEFT_JOIN)
    {
        return $this
            ->joinLabVisit($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'LabVisit', '\LabVisitQuery');
    }
